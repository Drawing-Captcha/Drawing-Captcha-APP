...
function deleteApiKey(e) {
    let tmpKey = e.apiKey
    let isDelete = true
    pushToServer(tmpKey, isDelete)
}

function pushToServer(key, isDelete) {
    fetch("/apiKey", {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        ...
        .then(data => {
            if (data.isGood) {
                alert("Changes successfully implemented on API system")
                location.reload()
            }
            else {
                alert("An error occurred, check the data processing of the API system in the backend")
                location.reload()
